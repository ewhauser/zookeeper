...
namespace ZooKeeperNet
{
  public class ZKPaths
  {
    ...
    {
      Mkdirs(zookeeper, path, true, CreateMode.Persistent);
    }

    /**
       * Make sure all the nodes in the path are ephemeral created. NOTE: Unlike File.mkdirs(), Zookeeper doesn't distinguish
       * between directories and files. So, every node in the path is created. The data for each node is an empty blob
       *
       * @param zookeeper the client
       * @param path      path to ensure
       ...
       *                              Zookeeper errors
       */
    public static void MakeEphemeralDirs(ZooKeeper zookeeper, String path)
    {
      Mkdirs(zookeeper, path, true, CreateMode.Ephemeral);
    }
     ...
     *                              Zookeeper errors
     */
    public static void Mkdirs(IZooKeeper zookeeper, String path, bool makeLastNode, CreateMode createMode)
    {
      PathUtils.ValidatePath(path);
      ...
      do
      {
        ...
        if (zookeeper.Exists(subPath, false) == null)
        {
          try
          {
            zookeeper.Create(subPath, new byte[0], Ids.OPEN_ACL_UNSAFE, createMode);
          }
